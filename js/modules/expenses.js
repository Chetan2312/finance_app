...
function renderEmiBody(sec) {
  ...
  S.debts.forEach(d => h += `<div class="sub-row"><span>${DICONS[d.type] || '🏦'}</span><div style="flex:1;min-width:0"><div class="sub-n">${d.name}<span class="lbdg">🔗 AUTO</span>${d.repay === 'interest' ? '<span class="bdg bdg-dan xxs">INT-ONLY</span>' : d.repay === 'bullet' ? '<span class="bdg bdg-warn xxs">BULLET</span>' : ''}</div><div class="sub-m">${d.rate}% p.a. · ${fmt(d.balance)}</div></div><span class="sub-a">${fmt(d.emi)}/mo</span></div>`);
  sec.items.filter(i => !i.linked).forEach(it => h += `<div class="sub-row"><span>📋</span><div style="flex:1;min-width:0"><div class="sub-n">${it.label}</div></div><div class="eirow" style="gap:3px"><span class="rp">₹</span><input class="einp" type="number" placeholder="0" min="0" data-sid="${sec.id}" data-iid="${it.id}" data-cust="false" value="${it.val || ''}" style="width:78px;background:transparent;border:none;color:var(--t1);font-family:var(--fm);font-size:.85rem;outline:none"><span class="xxs c-muted">/mo</span><button class="btn bd bsm bic" onclick="rmExpItem('${sec.id}','${it.id}',false)">✕</button></div></div>`);
